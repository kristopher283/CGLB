...
def predict_feats(args, model, bg, return_feats=False, return_node_feats=False):
    node_feats = bg.ndata[args['node_data_field']].cuda()
    if args.get('edge_featurizer', None) is not None:
        edge_feats = bg.edata[args['edge_data_field']].cuda()
        _, _, raw_feats, hidden_feats = model(bg, node_feats, edge_feats, return_feats=return_feats, return_node_feats=return_node_feats)
        return raw_feats.detach(), hidden_feats.detach()

    _, _, raw_feats, hidden_feats = model(bg, node_feats, return_feats=return_feats, return_node_feats=return_node_feats)
    return raw_feats.detach(), hidden_feats.detach()

class NET(torch.nn.Module):
    """
    Structure Loss model baseline for GCGL tasks
        ...
        if last_epoch:
            ...
            with torch.no_grad():
                for batch_id, batch_data in enumerate(data_loader[task_i]):
                    ...
                    raw_feats, hidden_feats = predict_feats(args, self.net, bg, return_feats=True)
                    bg = dgl.unbatch(bg)
